...
class CSVDataFeed(Feed):
    def __init__(self, file_name, price_field_name="Price Close", *args, **kwargs):
        super(CSVDataFeed, self).__init__(price_field_name=price_field_name, *args, **kwargs)
    ...
    def reset_feed(self, start_dt, end_dt):
        ...
        if (self.start_date != start_dt or self.end_date != end_dt) or (len(self.data) == 0):
    ...
    def get_price_data(self, end_dt, start_dt=None, offset=None):
        ...
        """
        prices = self.get_data(end_dt=end_dt, start_dt=start_dt, fields=self.price_field_name, offset=offset)
        return prices
